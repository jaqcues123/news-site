...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { truckId, filename, base64, mimeType } = body;
    ...
    const filePath = `public/images/trucks/${truckId}/${safeName}`;

    if (process.env.NODE_ENV === "development") {
      // Write directly to local filesystem in dev
      const fs = await import("fs/promises");
      const path = await import("path");
      const dir = path.join(process.cwd(), "public", "images", "trucks", truckId);
      await fs.mkdir(dir, { recursive: true });
      const buf = Buffer.from(base64, "base64");
      await fs.writeFile(path.join(dir, safeName), buf);
      return NextResponse.json({ url: `/images/trucks/${truckId}/${safeName}` });
    }
